...
function solution(s) {
  ...
  for (let i = 0; i < s.length; i++) {
    // 바로 앞을 확인해야 하기 때문에, index로 접근
    if (s[i] === "(") stack.push("("); // 1. 여는 괄호이면, stack에 넣기
    // 2. 닫는 괄호라면,
    else {
      stack.pop(); // 닫는 괄호와 짝꿍인 여는 괄호 하나 빼기

      // 1) 바로 앞이 여는 괄호라면, () 이니까 "레이저로 컷팅" -> 막대기 개수는 열려있는 괄호 갯수 === stack의 길이
      if (s[i - 1] === "(") answer += stack.length;
      // 2) 아니라면 (바로 앞이 여는 괄호가 아니고, 닫는 괄호라면), 막대기 끝나면서 조각 하나 생김 +1
